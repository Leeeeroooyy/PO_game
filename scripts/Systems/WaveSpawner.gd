...
const BASE_WAVE_COUNTS := {
	"line_melee": 2,
	"line_mage": 1,
	"line_siege": 0,
}
const RANGED_SPAWN_DELAY := 0.35
const SIEGE_SPAWN_DELAY := 0.7
const RANGED_ROW_SPACING := 56.0
const SIEGE_ROW_SPACING := 112.0
...
func get_next_wave_has_catapult() -> bool:
	return _get_wave_count_for_team(GameCatalog.TEAM_PLAYER, "line_siege") > 0


func is_catapult_wave(wave_number: int) -> bool:
	return wave_number > 0 and get_next_wave_has_catapult()
...
func _spawn_wave() -> void:
	...
	for lane in LANES:
		_spawn_team_wave(GameCatalog.TEAM_PLAYER, String(lane))
		_spawn_team_wave(GameCatalog.TEAM_ENEMY, String(lane))


func _spawn_team_wave(team: String, lane: String) -> void:
	for entry in _create_wave_formation(team):
		var formation_entry: Dictionary = entry
		_spawn_formation_entry(formation_entry, team, lane)


func _create_wave_formation(team: String) -> Array:
	var formation := []
	_add_formation_row(formation, "line_melee", _get_wave_count_for_team(team, "line_melee"), 0.0, 0.0)
	_add_formation_row(formation, "line_mage", _get_wave_count_for_team(team, "line_mage"), RANGED_ROW_SPACING, RANGED_SPAWN_DELAY)
	_add_formation_row(formation, "line_siege", _get_wave_count_for_team(team, "line_siege"), SIEGE_ROW_SPACING, SIEGE_SPAWN_DELAY)
	return formation


func _add_formation_row(formation: Array, unit_id: String, count: int, row_spacing: float, spawn_delay: float) -> void:
	for index in range(count):
		formation.append({
			"unit_id": unit_id,
			"lateral_amount": _formation_lateral_amount(index, count),
			"row_spacing": row_spacing,
			"spawn_delay": spawn_delay,
		})


func _spawn_formation_entry(entry: Dictionary, team: String, lane: String) -> void:
	var unit_id := String(entry.get("unit_id", ""))
	var lateral_amount := float(entry.get("lateral_amount", 0.0))
	var row_spacing := float(entry.get("row_spacing", 0.0))
	var spawn_delay := float(entry.get("spawn_delay", 0.0))
	if spawn_delay <= 0.0:
		_spawn_unit(unit_id, team, lane, lateral_amount, row_spacing)
		return
	...
	tree.create_timer(spawn_delay).timeout.connect(
		func() -> void:
			if _running and _actor_parent != null and _lane_unit_scene != null and _lane_manager != null:
				_spawn_unit(unit_id, team, lane, lateral_amount, row_spacing)
	)


func _get_wave_count_for_team(team: String, unit_id: String) -> int:
	var base_count := int(BASE_WAVE_COUNTS.get(unit_id, 0))
	if team != GameCatalog.TEAM_PLAYER:
		return base_count

	match unit_id:
		"line_melee":
			return mini(4, base_count + get_unit_upgrade_level("count_line_melee"))
		"line_mage":
			return mini(2, base_count + get_unit_upgrade_level("count_line_mage"))
		"line_siege":
			return mini(1, get_unit_upgrade_level("unlock_line_siege"))
		_:
			return base_count


func _spawn_unit(unit_id: String, team: String, lane: String, lateral_amount: float, row_spacing: float) -> void:
	var definitions := _get_unit_definitions()
	if not definitions.has(unit_id):
		return
	...
	if path.size() > 1:
		path = _build_formation_path(path, lateral_amount, row_spacing)
...
func _build_formation_path(path: PackedVector2Array, lateral_amount: float, row_spacing: float) -> PackedVector2Array:
...
func _formation_lateral_amount(index: int, count: int) -> float:
	match count:
		1:
			return 0.0
		2:
			return -20.0 if index == 0 else 20.0
		3:
			return (float(index) - 1.0) * 28.0
		4:
			return -42.0 + float(index) * 28.0
		_:
			return (float(index) - float(count - 1) * 0.5) * 28.0
